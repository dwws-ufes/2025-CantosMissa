...
@WebServlet(urlPatterns = { "/public/data/Artist/*" })
public class ArtistInRdfServlet extends HttpServlet {
    @EJB
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (pathInfo == null || pathInfo.equals("/")) {
            response.sendError(HttpServletResponse.SC_BAD_REQUEST, "É necessário informar o ID do artista.");
            return;
        }
        ...
        Artist artist = artistDAO.retrieveById(id);
        if (artist == null) {
            response.sendError(HttpServletResponse.SC_NOT_FOUND, "Artista não encontrado.");
            return;
        }
